...
# Ejecuta las tres versiones (serial, omp1, omp2) para medir rendimiento 
  # y genera un CSV con resultados robusto.
...
for N in "${POINTS_LIST[@]}"; do
    ...
    OUT_CSV="$OUT_DIR/${N}_results.csv"
    ...
    for REP in $(seq 1 $REPS); do
        # ---------------- Serial ----------------
        OUTPUT=$(./serial --in "$DATA" --eps "$EPS" --minpts "$MINPTS")
        ...
        for T in "${THREADS_LIST[@]}"; do
            OUTPUT=$(./omp1 --threads $T --in "$DATA" --eps "$EPS" --minpts "$MINPTS")
            TIME=$(echo "$OUTPUT" | grep "time_s=" | cut -d= -f2)
            CLUSTERS=$(echo "$OUTPUT" | awk '/clusters=/ {for(i=1;i<=NF;i++) if($i ~ /^clusters=/){split($i,a,"="); print a[2]}}')
            CORE=$(echo "$OUTPUT" | awk '/core=/ {for(i=1;i<=NF;i++) if($i ~ /^core=/){split($i,a,"="); print a[2]}}')
            BORDER=$(echo "$OUTPUT" | awk '/border=/ {for(i=1;i<=NF;i++) if($i ~ /^border=/){split($i,a,"="); print a[2]}}')
        ...
        # ---------------- omp2 ----------------
        for T in "${THREADS_LIST[@]}"; do
            OUTPUT=$(./omp2 --threads $T --in "$DATA" --eps "$EPS" --minpts "$MINPTS")
            TIME=$(echo "$OUTPUT" | grep "time_s=" | cut -d= -f2)
            CLUSTERS=$(echo "$OUTPUT" | awk '/clusters=/ {for(i=1;i<=NF;i++) if($i ~ /^clusters=/){split($i,a,"="); print a[2]}}')
            CORE=$(echo "$OUTPUT" | awk '/core=/ {for(i=1;i<=NF;i++) if($i ~ /^core=/){split($i,a,"="); print a[2]}}')
            BORDER=$(echo "$OUTPUT" | awk '/border=/ {for(i=1;i<=NF;i++) if($i ~ /^border=/){split($i,a,"="); print a[2]}}')
...
echo "✅ Experimentos completados. Resultados guardados en $OUT_DIR/"